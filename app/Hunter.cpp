...
void Hunter::chasePlayer(Player* p) {
	...
	if (p->position.getX() + 40 > this->position.getX()) {
		this->position.setX(this->position.getX() + (getDeltaTime() / 20.f));
	}
	...
	this->position.setY(this->position.getY() - 0.5*sin(getGlobalTime()/100));
}
